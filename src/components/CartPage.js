...
const CartPage = () => {
  ...
  const handleRemoveItem = (productInCartId) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/cart/item/${productInCartId}`, {
      method: 'DELETE',
    })
      .then((response) => {
        if (!response.ok) {
  ...
  const handleCancelAll = () => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/cart/${userId}/clear`, {
      method: 'DELETE',
    })
      .then((response) => {
        if (!response.ok) {
  ...
  const handlePayAll = () => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/order/${userId}/payall`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
