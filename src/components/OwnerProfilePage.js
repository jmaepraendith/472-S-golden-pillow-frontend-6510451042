...
function OwnerProfilePage() {
  ...
  useEffect(() => {
    const fetchIncomeSummary = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/income-summary`);
    ...
    const fetchSalesSummary = async () => {
      try {
        if (selectedMonth !== "Months") {
          ...
          if (month) {
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/sales-summary/${month}`);
